...
//        捏合手势缩放图片
        ...
        _cell.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width,  [UIScreen mainScreen].bounds.size.height);
        _cell.singleTapGestureBlock = ^(){
...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder completed:(dispatch_block_t)completed{
    ...
    [self sd_setImageWithURL:url placeholderImage:placeholder options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize,NSURL * targetURL) {
...
{
    if ((_totalScale < 1.0 && totalScale < _totalScale) || (_totalScale > 6.0 && totalScale > _totalScale)) return; // 最大缩放 2倍,最小0.5倍
