...
- (void)zwPlaceHolder_swizzling_layoutSubviews {
    if (self.zw_placeHolder) {
        if(self.placeHolderSizeFill){
            self.zw_placeHolderLabel.frame = self.frame;
        }else{
...
- (void)setPlaceHolderSizeFill:(BOOL)placeHolderSizeFill{
    objc_setAssociatedObject(self, &sizeFillKey, @(placeHolderSizeFill), OBJC_ASSOCIATION_COPY_NONATOMIC);
    [self setNeedsLayout];
}
- (BOOL)placeHolderSizeFill{
    NSNumber *sizeFill = objc_getAssociatedObject(self, &sizeFillKey);
    return [sizeFill boolValue];
}
