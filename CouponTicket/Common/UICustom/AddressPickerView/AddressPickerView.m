...
- (instancetype)init{
    if(self = [super initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight)]){
        ...
        UIView *showView = [[UIView alloc]initWithFrame:CGRectMake(0, ScreenHeight - 200 , ScreenWidth, 200 )];
        showView.backgroundColor = [UIColor whiteColor];
        [self addSubview:showView];
        
        UIView *sureView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, 46 )];
        [showView addSubview:sureView];
        UIButton *cancelBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [cancelBtn setFrame:CGRectMake(0, 0, 60 , sureView.bounds.size.height)];
        [cancelBtn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
        [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
        cancelBtn.titleLabel.font = [UIFont systemFontOfSize:14];
        [cancelBtn addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
        [showView addSubview:cancelBtn];
        UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake((ScreenWidth - 80)/2, 0, 80, sureView.bounds.size.height)];
        titleLabel.text = @"选择地区";
        titleLabel.textAlignment = NSTextAlignmentCenter;
        [showView addSubview:titleLabel];
        UIButton *sureBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [sureBtn setFrame:CGRectMake(ScreenWidth - 60 , 0, 60 , sureView.bounds.size.height)];
        [sureBtn setTitleColor:[UIColor colorWithRed:111/255.0 green:186/255.0 blue:39/255.0 alpha:1.0] forState:UIControlStateNormal];
        [sureBtn setTitle:@"确定" forState:UIControlStateNormal];
        sureBtn.titleLabel.font = [UIFont systemFontOfSize:14];
        [sureBtn addTarget:self action:@selector(sure) forControlEvents:UIControlEventTouchUpInside];
        [showView addSubview:sureBtn];
        
        _pickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(sureView.frame), ScreenWidth, 160 )];
...
- (NSAttributedString *)pickerView:(UIPickerView *)pickerView attributedTitleForRow:(NSInteger)row forComponent:(NSInteger)component{
    ...
    switch (component) {
        case 0:
            string = [[provinces objectAtIndex:row] objectForKey:@"state"];
            break;
        case 1:
            string = [[cities objectAtIndex:row] objectForKey:@"city"];
            break;
        case 2:
            if(areas.count > 0){
                string = [areas objectAtIndex:row];
                break;
            }
        default:
            string = @"";
    ...
    [attributedString addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]} range:NSMakeRange(0, string.length)];
...
- (void)setAddress:(Address *)address{
    ...
    [provinces enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if([[d objectForKey:@"state"] isEqualToString:address.province]){
            self->cities = [[provinces objectAtIndex:idx] objectForKey:@"cities"];
            provinceIndex = idx;
            *stop = YES;
        }
    }];
    [cities enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if([[d objectForKey:@"city"] isEqualToString:address.city]){
            self->areas = [[cities objectAtIndex:idx] objectForKey:@"areas"];
            cityIndex = idx;
            *stop = YES;
        }
