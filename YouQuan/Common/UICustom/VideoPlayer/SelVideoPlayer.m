...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    ...
    else if ([keyPath isEqualToString:@"status"])
    {
        if (self.player.currentItem.status == AVPlayerStatusReadyToPlay) {
            ...
            self.playerState = SelVideoPlayerStatePlaying;
            if(_delegate && [_delegate respondsToSelector:@selector(didFinishForPlayer:)]){
                [_delegate didFinishForPlayer:self];
            ...
            if(_delegate && [_delegate respondsToSelector:@selector(didFinishForPlayer:)]){
                [_delegate didFailedForPlayer:self];
...
- (SelPlaybackControls *)playbackControls
{
    if (!_playbackControls) {
        ...
        [_playbackControls.preImageView cl_setImageWithImg:_playerConfiguration.preImg];
