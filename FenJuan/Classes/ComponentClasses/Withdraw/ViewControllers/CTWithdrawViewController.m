...
@property (nonatomic, strong) UIButton *cashLogButton;
...
- (UIButton *)cashLogButton{
    if(!_cashLogButton){
        _cashLogButton = [UIButton buttonWithType:UIButtonTypeSystem];
        _cashLogButton.titleLabel.font = [UIFont systemFontOfSize:12];
        [_cashLogButton setTitle:@"提现记录" forState:UIControlStateNormal];
    }
    return _cashLogButton;
}
...
- (void)setUpUI{
    ...
    [self.autoLayoutContainerView addSubview:self.withdrawIntroButton];
    [self.autoLayoutContainerView addSubview:self.modifyView];
    [self.autoLayoutContainerView addSubview:self.cashLogButton];
}
...
- (void)autoLayout{
    ...
    [self.cashLogButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.modifyView.mas_bottom).offset(0);
        make.centerX.mas_equalTo(0);
        make.height.mas_equalTo(40);
    ...
    void (^__block withdrawActionBlock)(void) = ^{
        ...
        [CTAlertHelper showPayPasswordViewWithCallback:^(NSString *password) {
            ...
            [CTRequest cashSaveWithPaypwd:password money:self.viewModel.money callback:^(id data, CLRequest *request, CTNetError error) {
                ...
                else if([data[@"status"] integerValue] == 333){
...
#pragma clang diagnostic pop
    ...
    //提现记录
    [self.cashLogButton touchUpInsideSubscribeNext:^(id x) {
        @strongify(self)
        CTWithdrawHistoryViewController *vc = [CTWithdrawHistoryViewController new];
