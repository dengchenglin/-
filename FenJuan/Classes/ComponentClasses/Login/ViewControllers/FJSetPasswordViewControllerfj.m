...
#import "FJSetPasswordViewModelfj.h"

#import "CTNetworkEngine+Member.h"

@interface FJSetPasswordViewControllerfj ()

@property (nonatomic, strong) CTSetPasswordView *passwordView;

@property (nonatomic, strong) FJSetPasswordViewModelfj *viewModel;

@end

@implementation FJSetPasswordViewControllerfj
...
- (FJSetPasswordViewModelfj *)viewModel{
    if(!_viewModel){
        _viewModel = [FJSetPasswordViewModelfj new];
    }
    return _viewModel;
}
...
- (void)registerWithType:(CTLoginType)type{
    [CTRequest fj_registerWithPhone:self.mobile pwd:self.viewModel.password type:type ivCode:self.inviteCode smsCode:self.verCode openid:self.response.openid nickname:self.response.name headicon:self.response.iconurl unionid:self.response.unionId callback:^(id data, CLRequest *request, CTNetError error) {
        if(!error){
...
//找回密码
- (void)resetPwd{
    [CTRequest fj_resetPwdWithPhone:self.mobile pwd:self.viewModel.password smsCode:self.verCode callback:^(id data, CLRequest *request, CTNetError error) {
        if(!error){
...
- (void)resetWithDrawPwd{
    [CTRequest fj_setPaypwdWithPhone:self.mobile pwd:self.viewModel.password smsCode:self.verCode callback:^(id data, CLRequest *request, CTNetError error) {
        if(!error){
            [MBProgressHUD showMBProgressHudWithTitle:@"设置成功"];
