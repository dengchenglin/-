...
#import "FJTeamCateModel.h"

@interface FJMyTeamPageControllerfj ()<LMSegmentedControlDelegate>
@property (nonatomic, strong) LMSegmentedControl *segmentedControl;
@property (nonatomic, strong) FJTeamCateModel *model;
@end

@implementation FJMyTeamPageControllerfj
...
- (void)request{
    [CTRequest fj_teamCateWithCallback:^(id data, CLRequest *request, CTNetError error) {
        if(!error){
             self.model =  [FJTeamCateModel yy_modelWithDictionary:data];
            [self reloadView];
        }
    }];
}
...
- (void)reloadView{
    ...
    for(int i = 0;i < array.count;i ++){
        FJTeamListViewControllerfj *vc = [[FJTeamListViewControllerfj alloc]init];
