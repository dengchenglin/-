...
@interface FJMyCollectListViewControllerfj ()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic, strong) NSMutableArray <CTGoodsViewModel *> *dataSources;

@end

@implementation FJMyCollectListViewControllerfj
...
- (void)request{
    [CTRequest fj_myGoodsFavoriteWithPage:self.pageIndex size:self.pageSize callback:^(id data, CLRequest *request, CTNetError error) {
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    UITableViewRowAction *action1 = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault title:@"取消\n收藏" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        ...
        [CTRequest fj_favoriteWithGoodsId:viewModel.model.item_id isFavorite:NO callback:^(id data, CLRequest *request, CTNetError error) {
            if(!error){
