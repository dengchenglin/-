...
+ (void)createImagesWithBackgroundImgs:(NSArray <NSString *>*)backgroundImgs ivCodeImg:(NSString *)ivCodeImg ivCode:(NSString *)ivCode earn:(CTMyEarnModel *)earn completed:(void(^)(NSArray <UIImage *>*images))completed{
    ...
    for(NSString *backgroundImg in backgroundImgs){
        [self createImageWithBackgroundImg:backgroundImg ivCodeImg:ivCodeImg ivCode:ivCode earn:earn completed:^(UIImage *image) {
...
+ (void)createImageWithBackgroundImg:(NSString *)backgroundImg ivCodeImg:(NSString *)ivCodeImg ivCode:(NSString *)ivCode earn:(CTMyEarnModel *)earn completed:(void(^)(UIImage *image))completed{
    CTShareProfitView *imgView = NSMainBundleClass(CTShareProfitView.class);
    [imgView.userheadImageView sd_setImageWithURL:[NSURL URLWithString:[CTAppManager user].headimg]];
    imgView.usernameLabel.text = [CTAppManager user].nickname;
    imgView.userlevelLabel.text = [CTAppManager user].level_txt;
    imgView.qrCode = ivCode;
    imgView.profit = earn.all_money;
    imgView.balance = earn.valuation_money;
